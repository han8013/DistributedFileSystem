...
public class Membership {
    ...
    private static final int DETECTION_INTERVAL = 10000;
    ...
    public List<Member> getWorkerList() {
        ...
        System.out.print("Current workers: node ");
        for (Member member : list) {
            int id = Utils.ipToInt(member.getIp());
            if (id <= 7) {
                workerList.add(member);
                System.out.print(id + ", ");
            }
        }
        System.out.println();
        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    //gossip membership
                    if (MP4Server.flag) {
                        sendMessage(new Request("update", membershipList, 0), MP4Server.introduceIP);
        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    //heartbeats
                    if (MP4Server.flag) {
                        Member ownMember = membershipList.get(MP4Server.id);
                        ArrayList<Integer> targets = getHeartbeatTargets();
                        for (Integer target : targets) {
                            log.info("Heartbeat from " + MP4Server.id + " to " + target);
//                            System.out.println("XXXXXXXXXX Heartbeat from " + MP4Server.id + " to " + target);
...
//    }
    ...
    private ArrayList<Integer> getHeartbeatSenders() {
        ...
        for (Member member : list) {
            ...
            if (member.getId() > MP4Server.id) {
                array[index] = member.getId() - MAX_NODES;
                index++;
            } else if (member.getId() < MP4Server.id) {
...
//                removeList.remove(member.getId());
    ...
    public void updateMembershipList(TreeMap<Integer, Member> incoming) {
        ...
        SDFSServer.fileManager.updateFileList(membershipList);
        if (size == 1 && incoming.size() > 2 && MP4Server.myIP.equals(MP4Server.introduceIP)) {
            gossipSend(new Request("update", membershipList, TTL));
        }
    }

    public void leaveCluster() {
        Request request = new Request("leave", membershipList.get(MP4Server.id), TTL);
        gossipSend(request);
    ...
    public void gossipSend(Request request) {
        ...
        ArrayList<Member> list = copyMembershipList(membershipList);
        list.remove(membershipList.get(MP4Server.id));
        if (list.size() >= 1) {
            log.info("gossipSend method run.");
            for (int i = 0; i < 4 && list.size() > 0; i++) {
                int index = (int) (Math.random() * list.size());
                log.info("gossip send from" + MP4Server.id + "to" + list.get(index).getId() + "action: " + request.getAction());
