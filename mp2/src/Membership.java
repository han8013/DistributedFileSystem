        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    //gossip membership
                    if (MP2Server.flag) {
                        sendMessage(new Request("update", membershipList, 0), MP2Server.introduceIP);
        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    //heartbeats
                    if (MP2Server.flag) {
                        Member ownMember = membershipList.get(MP2Server.id);
                        ArrayList<Integer> targets = getHeartbeatTargets();
                        for (Integer target : targets) {
                            log.info("Heartbeat from " + MP2Server.id + " to " + target);
    ...
    private ArrayList<Integer> getHeartbeatSenders() {
        ...
        for (Member member : list) {
            ...
            if (member.getId() > MP2Server.id) {
                ...
                index++;
            } else if (member.getId() < MP2Server.id) {
...
//                removeList.remove(member.getId());
    ...
    public void updateMembershipList(TreeMap<Integer, Member> incoming) {
        ...
        if (size == 1 && incoming.size() > 2 && MP2Server.myIP.equals(MP2Server.introduceIP)) {
            gossipSend(new Request("update", membershipList, TTL));
        }
    }

    public void leaveCluster() {
        Request request = new Request("leave", membershipList.get(MP2Server.id), TTL);
        gossipSend(request);
    ...
    public void gossipSend(Request request) {
        ...
        ArrayList<Member> list = copyMembershipList(membershipList);
        list.remove(membershipList.get(MP2Server.id));
        if (list.size() >= 1) {
            log.info("gossipSend method run.");
            for (int i = 0; i < 4 && list.size() > 0; i++) {
                int index = (int) (Math.random() * list.size());
                log.info("gossip send from" + MP2Server.id + "to" + list.get(index).getId() + "action: " + request.getAction());
