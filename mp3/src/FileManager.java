...
public class FileManager {
    ...
    public byte[] readFile(String filename) {
        for (SDFSFile file : fileLists.get(Utils.ipToInt(MP3Server.myIP))) {
    ...
    public void updateFileList(TreeMap<Integer, Member> membershipList) {
        ...
        for (Integer key : membershipList.keySet()) {
            if (fileLists.get(key) == null) {
                ...
                int myKey = Utils.ipToInt(MP3Server.myIP);
                if (key != myKey) {
    ...
    private void backup() {
        ...
        for (String filename : filenames) {
            ...
            if (keys.size() < 3 && keys.size() > 0) {
                ...
                if (minKey == Utils.ipToInt(MP3Server.myIP)) {
                    ...
                    SDFSFile file = null;
                    for (SDFSFile temp : fileLists.get(Utils.ipToInt(MP3Server.myIP))) {
    ...
    private Response sendPackage(String ip, Request request) {
    ...
    public String getDescription() {
        ...
        for(int key : fileLists.keySet()) {
            if (Utils.ipToInt(MP3Server.myIP) == key) {
                result.append("[self]");
            } else {
                result.append("      ");
            }
    ...
    public void sendFileList(int key) {
        sendPackage(Utils.intToIP(key), new Request("fileList", fileLists.get(Utils.ipToInt(MP3Server.myIP))));
    }
}
